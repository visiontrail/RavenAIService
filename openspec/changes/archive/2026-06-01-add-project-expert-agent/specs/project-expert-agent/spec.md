...
- **WHEN** a Project Expert run starts with one enabled skill `foo`
- **THEN** `<workspace>/.claude/skills/foo/` exists before the `query()` loop begins
