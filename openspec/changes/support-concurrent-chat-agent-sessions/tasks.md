...
- [x] 10.1 后端测试集：`tests/api/test_chat_happy_path.py`、DeviceAgent tests、HITL tests、LogAnalysis chat stream tests 通过 — `test_chat_happy_path.py` 与其它 26 个 chat 测试全部通过（含 `_run_device_job` 的 `done` payload 补回 `messages` 字段）；`test_chat_hitl_integration.py::test_chat_stream_full_hitl_flow`、`test_log_analysis_chat_stream_trace.py` 中 2 个 reconnect-replay 用例在主分支上即已超时失败，与本变更无关，单独跟踪
