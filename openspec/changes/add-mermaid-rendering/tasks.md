...
- [ ] 6.1 验证基本渲染：flowchart、sequence、class、state、ER、gantt、pie 等图表类型在聊天中正确渲染为 SVG
- [ ] 6.2 验证错误降级：发送含语法错误的 Mermaid 代码块，确认显示高亮源码和错误提示
- [ ] 6.3 验证流式渲染：观察 AI 流式回复过程中 Mermaid 代码块的行为（未完成时不闪烁，完成后自动渲染）
- [ ] 6.4 验证交互功能：点击图表放大查看、复制源码功能正常
- [ ] 6.5 验证性能：首次加载页面时 Mermaid 库未被加载（查看 Network），仅在遇到 Mermaid 代码块后才动态加载
