...
- Split trace semantics into all configured/enabled Skills and Skills actually invoked through the `Skill` tool, without materializing the whole catalog.
...
- `agent-trace-stream`: Report the full enabled catalog independently from filtered materialization and actual Skill invocations.
