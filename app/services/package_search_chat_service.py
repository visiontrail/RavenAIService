...
class PackageSearchChatService:
        ...
        try:
            ...
            if loaded is None:
                ...
                yield self._sse_event(
                    {
                        "event": "package_search_status",
                        "message": "已复用当前对话的重构包检索工作区，正在继续分析...",
                    }
                )
            ...
            yield self._sse_event(
                {
                    "event": "package_search_status",
                    "message": "Package Search Agent 正在检索项目的重构包...",
                }
            )
    ...
    async def _run_job_async(self, job: AgentJob, ctx: WorkspaceContext) -> None:
        ...
        try:
            ...
            result = await asyncio.to_thread(
                PackageSearchAgent().run_sync, ctx, job.cancel_event, _emit_trace
            )
            job.result = result
            ...
            job.events.append(
                {
                    ...
                    "recommended_package_ids": result.get("recommended_package_ids") or [],
    ...
    async def _persist_job_result(
        ...
        if db_manager.session_factory is None:
            logger.warning("package-search chat: db not initialized; skipping persistence")
            return
    ...
    def _load_context(
        ...
        *,
        ...
        if not ignore_owner and owner_user_id and getattr(user, "id", None) != owner_user_id:
            raise PermissionError("当前用户无权访问这个重构包检索上下文")
    ...
    def _format_agent_result(
        ...
        *,
        ...
        if relevant:
            lines.append(
                "- 相关包：" + " ".join(f"`{str(pid)}`" for pid in relevant)
            )

        if answer:
            lines.extend(["", "## 回答", answer])
        elif status == "ok":
            lines.extend(["", "## 回答", "模型没有返回可展示的回答内容。"])
