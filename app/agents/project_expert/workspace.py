...
class WorkspaceContext:
    ...
    与 log_analysis 的 ``WorkspaceContext`` 同构，但 **不含 ``logs_dir``** ——
    本智能体不处理日志归档。
    """
...
def prepare(
    *,
    ...
    if project_repo is None:
        raise MissingProjectRepoError("project_repo is required for Project Expert workspace")
    ...
    try:
        ...
        logger.info(
            "Project Expert workspace prepared: task_id=%s temp_dir=%s project_code=%s session_id=%s",
...
def cleanup(ctx: WorkspaceContext) -> None:
    ...
    if temp.exists():
        ...
        logger.info("Project Expert workspace cleaned up: %s", ctx.temp_dir)
